<?php
/**
 * This is extended upon the Elgg wire plugin that was extended upon by the wire with access. 
 *  The following created the wire and the wire with access.
 ...
 * Max Thomas
 * Nathan Koterba
 The following are repsonsible for the PZ wire
 Ryan Satterfield is repsonsible for PZ wire; except for adding friends into the widget, which Matt Beckett is responsible for.  
 */
...
function thewire_init() {
	...
	// don't have this Register for search
...
/*** getting friends guids and usernames in one command!

**/
function get_friend_info()
{
$friend = get_user_by_username((string)get_input('friend'));
...
function thewire_save_post($text, $userid, $access_id, $parent_guid = 0, $method = "site") {
	...
	$post->description = htmlentities($text, ENT_NOQUOTES, 'UTF-8');
